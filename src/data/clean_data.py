...
def replace_illogical(train, test):
    '''Takes in the training and testing dataframes and cleans illogical values
    from feature folumns.
    Input: 2 pandas dataframes
    Output: 2 pandas dataframes'''
...
def lowercase_features(train, test):
    '''Takes in the training and testing dataframes and makes all text lowercase
    to ensure values that are meant to be the same are recognized as such.
    Input: 2 pandas dataframes
    Output: 2 pandas dataframes'''

    for df in [train, test]:
        for col in df:
            if df[col].dtype == 'O' and col not in\
...
def fill_construction_year(train, test):
    '''Fills nans in construction year column by creating a random forest
    regressor with construction year as the target.
    Input: 2 pandas dataframes
    Output: 2 pandas dataframes'''

    dummies_train = pd.get_dummies(train[['basin', 'extraction_type_class',
                                          'management', 'water_quality',
                                          'quantity', 'source']])
    train = pd.concat([train, dummies_train], axis=1)
    clean_constr_train = train.drop(['funder', 'installer', 'wpt_name',
                                     ...
                                     'quantity',
                                     'source', 'source_type',
                                     'waterpoint_type', 'recorded_by',
                                     'extraction_type_group',
                                     'extraction_type_class', 'quantity_group',
                                     'source_class', 'waterpoint_type_group',
                                     'region_code', 'district_code',
                                     'status_group', 'id', 'population'],
                                    axis=1)

    dummies_test = pd.get_dummies(test[['basin', 'extraction_type_class',
                                        'management', 'water_quality',
                                        'quantity', 'source']])
    test = pd.concat([test, dummies_test], axis=1)
    clean_constr_test = test.drop(['funder', 'installer', 'wpt_name', 'basin',
                                   ...
                                   'source_class', 'waterpoint_type_group',
                                   'region_code', 'district_code',
                                   'id', 'population'], axis=1)

    constr_y_train = clean_constr_train[~clean_constr_train
                                        ['construction_year']
                                        .isnull()]['construction_year']
    constr_y_test = clean_constr_test[~clean_constr_test
                                      ['construction_year']
                                      .isnull()]['construction_year']
    constr_y_total = pd.concat([constr_y_train, constr_y_test],
                               axis=0, ignore_index=True).values

    constr_X_train = clean_constr_train[~clean_constr_train
                                        ['construction_year'].isnull()][
                                        ['amount_tsh', 'gps_height',
                                         'longitude', 'latitude',
                                         'num_private', 'public_meeting',
                                         'permit']]
    constr_X_test = clean_constr_test[~clean_constr_test
                                      ['construction_year'].isnull()][
                                      ['amount_tsh', 'gps_height', 'longitude',
                                       'latitude', 'num_private',
                                       'public_meeting', 'permit']]
    constr_X_total = pd.concat([constr_X_train, constr_X_test],
                               axis=0, ignore_index=True).values

    constr_X_train, constr_X_test, constr_y_train,\
    constr_y_test = train_test_split(constr_X_total, constr_y_total)

    model_constr = RandomForestRegressor()
    model_constr.fit(constr_X_train, constr_y_train)
    mse = mean_squared_error(constr_y_test,
                             model_constr.predict(constr_X_test))
    # print np.sqrt(mse)

    prediction_df_train = train[train['construction_year'].isnull()]
    train_preds = model_constr.predict(
                                       prediction_df_train[['amount_tsh',
                                                            'gps_height',
                                                            'longitude',
                                                            'latitude',
                                                            'num_private',
                                                            'public_meeting',
                                                            'permit']].values)
    train.ix[train['construction_year'].isnull(),
             'construction_year'] = train_preds

    prediction_df_test = test[test['construction_year'].isnull()]
    test_preds = model_constr.predict(
                                      prediction_df_test[['amount_tsh',
                                                          'gps_height',
                                                          'longitude',
                                                          'latitude',
                                                          'num_private',
                                                          'public_meeting',
                                                          'permit']].values)
    test.ix[test['construction_year'].isnull(),
            'construction_year'] = test_preds
...
def make_naive_bayes(train, test):
    '''Aggregates the text columns in the dataframe and creates a naive bayes
    classifier using the text as the input.  Naive bayes predictions then
    become a new feature.
    Input: 2 pandas dataframes
    Output: 2 pandas dataframes'''

    train['text_cols'] = train[['funder', 'wpt_name', 'subvillage', 'region',
                                'lga', 'ward', 'scheme_management',
                                'scheme_name',
                                'extraction_type', 'management_group',
                                'quality_group',
                                'waterpoint_type']].values.tolist()

    train.drop(['funder', 'installer', 'wpt_name', 'basin', 'subvillage',
                'region', 'lga', 'ward', 'scheme_management', 'scheme_name',
                'extraction_type', 'management', 'management_group', 'payment',
                'payment_type', 'water_quality', 'quality_group', 'quantity',
                'source', 'source_type', 'waterpoint_type', 'recorded_by',
                'extraction_type_group', 'extraction_type_class',
                'quantity_group', 'source_class', 'waterpoint_type_group',
                'region_code', 'district_code'], axis=1, inplace=True)

    test['text_cols'] = test[['funder', 'wpt_name', 'subvillage', 'region',
                              'lga', 'ward', 'scheme_management',
                              'scheme_name', 'extraction_type',
                              'management_group', 'quality_group',
                              'waterpoint_type']].values.tolist()
    test.drop(['funder', 'installer', 'wpt_name', 'basin', 'subvillage',
               'region', 'lga', 'ward', 'scheme_management', 'scheme_name',
               'extraction_type', 'management', 'management_group', 'payment',
               'payment_type', 'water_quality', 'quality_group', 'quantity',
               'source', 'source_type', 'waterpoint_type', 'recorded_by',
    ...
    train['text_cols'] = train['text_cols'].map(lambda x: ' '.join(str(word)
                                                for word in x))
    test['text_cols'] = test['text_cols'].map(lambda x: ' '.join(str(word)
                                                                 for word in
                                                                 x))

    naive_y = train['status_group'].values
    naive_X = train['text_cols'].values
    naive_X_train, naive_X_test, naive_y_train, \
    naive_y_test = train_test_split(naive_X, naive_y, stratify=naive_y)

    model_snow, vect_snow = make_model(tokenize_snow,
                                       naive_X_train, naive_y_train)

    train['naive_bayes'] = model_snow.predict(vect_snow.transform(naive_X))
    test['naive_bayes'] = model_snow.predict(vect_snow.transform
                                             (test['text_cols'].values))
